...
class ParseLogs:
    ...
    def parse_damage_info(self, parts: list, line_index: int):
        pokemon_spot = parts[2].split()[0]
        target_player = self.find_player(pokemon_spot)
        pokemon = self.turns[self.current_turn]['battle_field_state_end'][target_player][pokemon_spot[2]]
        damage = parts[3].split('/')[0]
        if len(parts) == 4:
            atacking_mon = self.get_attacking_mon(line_index)
        self.turns[self.current_turn]['moves'].append(
            {
                'player': target_player,
                'action': 'damage',
                'actor': atacking_mon or '',
                'target': pokemon,
                'damage': damage,
                'description': f"{pokemon} took {damage} damage."
            }
        )

    def get_attacking_mon(self, line_index: int):
        for i in range(line_index, -1, -1):
            line = self.lines[i]
            if line.startswith('|move|'):
                parts = line.split('|')
                pokemon_spot = parts[2].split()[0]
                player = self.find_player(pokemon_spot)
                return self.turns[self.current_turn]['battle_field_state_end'][player][pokemon_spot[2]]
        ...
        self.turns[self.current_turn]['moves'].append(
            {
                ...
                'action': 'crit',
                'target': pokemon,
                'description': f"{pokemon} received a critical hit."
            }
        )
        ...
        for index, line in enumerate(self.lines):
            ...
            if self.current_turn == 0:
                ...
                if line.startswith('|turn|'):
                    ...
                    self.parse_damage_info(line.split('|'), index)
                else:
                    self.parse_action_info(line)
